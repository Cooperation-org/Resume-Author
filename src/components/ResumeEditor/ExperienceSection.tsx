...
const formatToWorkExperience = (formData: ExperienceFormData): FullWorkExperience => ({
  id: formData.id,
...
const formatFromWorkExperience = (exp: FullWorkExperience): ExperienceFormData => ({
  id: exp.id,
  company: exp.employmentHistoryItem.organization.tradeName,
  domainName: exp.employmentHistoryItem.organization.domainName ?? '',
  title: exp.employmentHistoryItem.title,
  city: exp.employmentHistoryItem.location.city || '',
  postalCode: exp.employmentHistoryItem.location.postalCode ?? '',
  countryCode: exp.employmentHistoryItem.location.countryCode ?? 'US',
  unitName: exp.employmentHistoryItem.organizationUnit.name ?? '',
  duration: exp.employmentHistoryItem.duration,
  jobLevels:
    exp.employmentHistoryItem.jobLevels.length > 0
      ? exp.employmentHistoryItem.jobLevels.map((level, index) => ({
          id: `joblevel-${exp.id}-${index}`,
          name: level.name
        }))
      : [{ id: `joblevel-${exp.id}-0`, name: '' }]
})
...
const ExperienceSection: React.FC = () => {
  ...
  const [viewingExp, setViewingExp] = useState<FullWorkExperience | null>(null)
  ...
  const handleEdit = useCallback((experience: FullWorkExperience) => {
  ...
  const handleView = (experience: FullWorkExperience) => {
