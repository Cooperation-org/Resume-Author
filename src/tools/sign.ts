...
const signResume = async ({resume, accessToken}:{resume: Resume, accessToken: string}) => {
    let keyPair
    ...
    const keyPairs = await storage.getAllFilesByType('KEYPAIRs') as unknown as {data: any}[]
